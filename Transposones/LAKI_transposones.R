...
#crea el rank a partir del df del resultado DESeq
get_rank<-function(x){
...
#intersecta el term to gene y el rank y después ejecuta la función GSEA, x=rank
GSEA_transp<-function(x){
  term_to_gene<-term_to_gene[term_to_gene$gene_name %in% names(x),]#solo los transposones con lecturas en el experimento
...
#GSEA DE LAS FAMILIAS DE TRANSPOSONES
...
#creamos una semilla para obtener siempre los mismos resultados
set.seed(10)
