...
load("/home/antotartier/data/histones/LAKI/LFC_hist_sig.obj")
load("/home/antotartier/data/histones/LAKI/LFC_hist_M_sig.obj")
load("/home/antotartier/data/histones/LAKI/LFC_hist_F_sig.obj")

#juntamos los datos de histonas y velocidad
correl_sig<-merge(LFC_tissue,LFC_hist_sig,by="tejido")
correl_M_sig<-merge(LFC_tissue_M,LFC_hist_M_sig,by="tejido")
correl_H_sig<-merge(LFC_tissue_H,LFC_hist_H_sig,by="tejido")

#representamos los datos
plot_correl_sig<-ggplot(correl_sig,aes(x=LFC,y=media))+
  geom_point(aes(color=tejido))+
  geom_smooth(method = lm,)+
  stat_poly_eq(size=3)+
  labs(x="LFC medio de las histonas",y="Velocidad de la elongación media")+
...
plot_correl_M_sig<-ggplot(correl_M_sig,aes(x=LFC,y=media))+
  geom_point(aes(color=tejido))+
  geom_smooth(method = lm,)+
  stat_poly_eq(size=3)+
  labs(x="LFC medio de las histonas",y="Velocidad de la elongación media")+
...
plot_correl_H_sig<-ggplot(correl_H_sig,aes(x=LFC,y=media))+
  geom_point(aes(color=tejido))+
  geom_smooth(method = lm,)+
  stat_poly_eq(size=3)+
  labs(x="LFC medio de las histonas",y="Velocidad de la elongación media")+
...
#guardamos el resultado
png(file="/home/antotartier/data/velocidad_transcrip/LAKI/Correl_speed_Hist_sig.png",width = 20000, height = 5000, res = 1100)
grid.arrange(plot_correl_sig,plot_correl_M_sig,plot_correl_H_sig,ncol=3)
dev.off()