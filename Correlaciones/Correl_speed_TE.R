...
#----------------------------------------------------------------------------------------------------------------------------------------
#Lo repetimos con los resultados estadísticamente significativos
LFC_mean_sig<-function(x,tis){
  z<-na.omit(x)
  z<-z[z$padj<0.05,]
  y<-mean(z$log2FoldChange)
  LFC<-data.frame(LFC_TE=y,tejido= tis)
}

LFC_TE_all_sig<-mapply(LFC_mean_sig,TE_all,tis,SIMPLIFY = F)
LFC_TE_all_sig<-do.call(rbind,LFC_TE_all_sig)

LFC_TE_M_sig<-mapply(LFC_mean_sig,TE_M,tis,SIMPLIFY = F)
LFC_TE_M_sig<-do.call(rbind,LFC_TE_M_sig)

LFC_TE_F_sig<-mapply(LFC_mean_sig,TE_F,tis,SIMPLIFY = F)
LFC_TE_F_sig<-do.call(rbind,LFC_TE_F_sig)

#juntamos los datos de histonas y velocidad
correl_sig<-merge(LFC_tissue,LFC_TE_all_sig,by="tejido")
correl_M_sig<-merge(LFC_tissue_M,LFC_TE_M_sig,by="tejido")
correl_F_sig<-merge(LFC_tissue_H,LFC_TE_F_sig,by="tejido")

#representamos los datos
plot_correl_sig<-ggplot(correl_sig,aes(x=LFC_TE,y=media))+
  geom_point(aes(color=tejido))+
  geom_smooth(method = lm,)+
  stat_poly_eq(size=3)+
  labs(x="LFC medio de los transposones",y="Velocidad de la elongación media")+
...
plot_correl_M_sig<-ggplot(correl_M_sig,aes(x=LFC_TE,y=media))+
  geom_point(aes(color=tejido))+
  geom_smooth(method = lm,)+
  stat_poly_eq(size=3)+
  labs(x="LFC medio de los transposones",y="Velocidad de la elongación media")+
...
plot_correl_F_sig<-ggplot(correl_F_sig,aes(x=LFC_TE,y=media))+
  geom_point(aes(color=tejido))+
  geom_smooth(method = lm,)+
  stat_poly_eq(size=3)+
  labs(x="LFC medio de los transposones",y="Velocidad de la elongación media")+
...
#guardamos el resultado
png(file="/home/antotartier/data/velocidad_transcrip/LAKI/Correl_speed_TE_sig.png",width = 20000, height = 5000, res = 1100)
grid.arrange(plot_correl_sig,plot_correl_M_sig,plot_correl_F_sig,ncol=3)
dev.off()